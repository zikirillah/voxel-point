...
def placeholder_inputs():
    the_model_ph = tf.placeholder(tf.float32, shape=(model.N, model.N, model.N, model.NUM_FEATURES))
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(FLAGS.gpu)):
            ...
            enqueue_op = queue.enqueue([the_model_ph, cat_label_ph, seg_label_ph])
            dequeue_the_model, dequeue_cat_label, dequeue_seg_label = queue.dequeue_many(batch_size)

            # model
            pred_cat, pred_seg = model.get_model(dequeue_the_model, is_training=is_training_ph)
        ...
        for num_thread in range(16):
            t = StoppableThread(target=load_and_enqueue, args=(sess, enqueue_op, the_model_ph, cat_label_ph, seg_label_ph))
            t.setDaemon(True)
            t.start()
